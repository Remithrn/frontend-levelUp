...
import {
  ...
  fetchallUsers,
  getSalesDetails,
} from "../features/customAdmin/customAdminSlice"; // Updated import, removed getTotalSales
...
const UserList = ({ users }) => {
  ...
  const totalExperiencePoints = userList.reduce(
    (total, user) => total + user.experience_points,
    0,
  );
  const averageStreakLength =
    totalUsers > 0
      ? (
          userList.reduce((total, user) => total + user.streak_length, 0) /
          totalUsers
        ).toFixed(2)
      : 0;
  const subscribedUsers = userList.filter(
  ...
  return (
    <div className="user-list">
      ...
      <div className="overall-statistics mb-4 p-4 border border-gray-300 rounded">
        ...
        <p>
          <strong>Inactive Users:</strong> {inactiveUsers}
        </p>
        <p>
          <strong>Subscribed Users:</strong> {subscribedUsers}
        </p>
        <p>
          <strong>Total Experience Points:</strong> {totalExperiencePoints}
        </p>
        <p>
          <strong>Average Streak Length:</strong> {averageStreakLength}
        </p>
      </div>
      ...
      {userList.map((user) => (
        <div
          key={user.username}
          className="user-card flex justify-between items-center"
          style={{
        ...
        <ModalContent>
            ...
            {selectedUser && (
              <>
                ...
                <ul className="list-disc list-inside">
                  <li>
                    <strong>Streak Length:</strong> {selectedUser.streak_length}
                  </li>
                  <li>
                    <strong>Total Experience Points:</strong>{" "}
                    {selectedUser.experience_points}
                  </li>
                  <li>
                    <strong>AI Tokens:</strong> {selectedUser.ai_tokens}
                  </li>
                </ul>
                <p>
          ...
          <ModalFooter>
            {selectedUser && (
              <Button
                color="primary"
                onClick={() => handleDeactivateUser(selectedUser.username)}
              >
